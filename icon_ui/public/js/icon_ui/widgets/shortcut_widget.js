...
export default class ShortcutWidget extends Widget {
	...
	get_config() {
		return {
			name: this.name,
			custom_app_icon: this.custom_app_icon,
			custom_icon_color: this.custom_icon_color,
			label: this.label,
			format: this.format,
	...
	set_count(count) {
		...
		if(this.custom_app_icon && this.custom_app_icon != null && this.custom_app_icon != ''){
			$(`.shortcut-widget-box`).css({"align-items": "center"})
			$(
				`<div class="count-notify">&nbsp;${label}&nbsp;</div>`
			).appendTo(this.icon_widget);
		}else{
			$(`.shortcut-widget-box`).css({"align-items": "flex-start"})
			$(
